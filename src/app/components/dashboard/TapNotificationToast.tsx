...
export function TapNotificationToast() {
  ...
  const channelRef = useRef<ReturnType<ReturnType<typeof getViteSupabaseClient>["channel"]> | null>(null);
  ...
  useEffect(() => {
    ...
    async function subscribe() {
      ...
      channelRef.current = db.channel("tap_notifications_vite")
        .on(
          "postgres_changes",
          {
            event: "INSERT",
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 flex flex-col gap-2 max-w-xs w-full">
      {toasts.map((toast) => (
        <div
          key={toast.id}
          className="flex items-start gap-3 rounded-2xl border border-[var(--border-muted)] bg-[var(--bg-secondary)] p-4 shadow-xl shadow-black/40"
        >
