...
function MetricBadge({ label, value, icon: Icon }: { label: string; value: number; icon: React.ElementType }) {
  return (
      ...
      <div className="mx-auto max-w-lg space-y-4">
        ...
        >
          <div className="grid grid-cols-3 gap-2 sm:grid-cols-6">
            {[
              { label: "Save", icon: Download, href: `/save/${profile.username}` },
              {
                ...
                .map((link) => {
                  ...
                  return (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="floating-card flex items-center gap-2.5 rounded-xl border border-[var(--border-muted)] bg-[var(--bg-elevated)] px-3 py-2.5 text-sm text-[var(--text-secondary)] capitalize hover:text-[var(--text-primary)]"
                    >
